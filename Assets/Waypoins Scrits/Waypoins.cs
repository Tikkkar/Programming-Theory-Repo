...
public class Waypoins : MonoBehaviour
{
    ...
    [Header("PartSetting")]
    //Sets the path to be looped so agent will go from last waypoint to the fist or vice versa
    [SerializeField] private bool canLoop = true;


    // Sets the agent to move for ward or backwards
    ...
    private void OnDrawGizmos()
    {
        ...
        // If the path is set to loop then drawn a line between the last and first waypoint
        if (canLoop)
        {
    ...
    public Transform GetNextWaypoint(Transform currentWaypoint)
    {
        ...
        if (isMovingForward)
        {
            ...
            // Check if that path is set to loop and return the first waypoint as the current waypoint, otherwise we subtract1
            // from nextIndex which will return the same waypoint that the agent is currently at, wich will cause it to stop
        ...
        else
        {
            ...
            // if the next In dex is below 0 then it means that you are already at the first waypoint, check if the path is set
            // to loop and if so return the last waypoint, otherwise we add 1 to the nextIndex wich will return the current
        ...
        // Return the waypoint that has an index of next Index
